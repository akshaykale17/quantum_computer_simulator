...
def get_operator(total_qubits, gate_unitary, target_qubits,params,global_params):
    ...
    if(gate_unitary=='u3'):
        if(type(params['theta'])==str):
            theta = global_params[params['theta']]
        else:
            theta = params['theta']
        if(type(params['phi'])==str):
            phi = global_params[params['phi']]
        else:
            phi = params['phi']
        if(type(params['lamb'])==str):
            lamb = global_params[params['lamb']]
        else:
            lamb = params['lamb']
        i = complex(0+1j)
        a1 = math.cos(theta/2)
        ...
        a4 = cmath.exp(i*lamb+i*phi)*math.cos(theta/2)

        cached_gates['u3'] = np.array([
        [round_complex_numbers(a1),round_complex_numbers(a2)],
        [round_complex_numbers(a3),round_complex_numbers(a4)]
        ])
...
def run_program(total_qubits,initial_state, program,global_params={}):

    for i in range(len(program)):
        initial_state = np.dot(initial_state,get_operator(total_qubits,program[i]['gate'],program[i]['target'],program[i]['params'],global_params))
